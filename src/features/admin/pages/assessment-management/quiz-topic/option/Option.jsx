...
const Option = () => {
  ...
  const { questionId } = useParams(); // 👈 change

  const { options, isLoading, isError, message } = useSelector(
    (state) => state.option, // 👈 change
  );
  ...
  // Fetch options
  const fetchOptions = () => {
    dispatch(getAllOptions(questionId));
  };
  ...
  //   if (window.confirm(t("option.confirmDelete"))) {
  //     try {
  ...
  //   },
  //   {
  //     header: t("option.columns.optionText"),
  //     render: (row) => (
  ...
  // if (isLoading && !options?.length) return <Loader />;
      ...
      <PageBody>
        <div className="mt-4">
          <CustomeTable
            ...
            totalPages={options?.last_page || 1}
