  ...
  const onSubmit = async (values, { setSubmitting, setErrors, resetForm }) => {
    try {
      ...
      const res = await dispatch(
        createQuestion({ assessmentId: assessmentId, data: formData }),
      ).unwrap();

      toast.success(res?.message || t("question.success.create"));
      resetForm();
      removeThumbnail();
      navigate(`/assessment-question/${assessmentId}`);
    } catch (error) {
      setErrors({ submit: error.message });
      toast.error(error?.message || t("question.error.create"));
    } finally {
      setSubmitting(false);
    }
  };
