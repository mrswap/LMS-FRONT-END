...
const QuestionDetails = () => {
  ...
  useEffect(() => {
    if (question?.file) {
      setThumbnailPreview(question.file);
    }
  }, [question]);
  ...
  const removeThumbnail = () => {
    setThumbnail(null);
    setThumbnailPreview(question?.file || null);
  ...
  const onSubmit = async (values, { setSubmitting, setErrors }) => {
    try {
      ...
      if (thumbnail && thumbnail !== question?.file) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.error(error?.message || t("question.error.delete"));
    }
  };
