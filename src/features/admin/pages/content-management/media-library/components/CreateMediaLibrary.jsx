...
const CreateMediaLibrary = () => {
  ...
  const typeOptions = [
    ...
    { label: "H5P", value: "h5p" }, // Added H5P option
  ];
  ...
  const getValidationSchema = () => {
    ...
    } else if (selectedType?.value === "h5p") {
      // H5P only requires external URL, no file upload
      schema = schema.shape({
        externalUrl: Yup.string()
          .url(t("mediaLibrary.validation.invalidUrl"))
          .required(t("mediaLibrary.validation.urlRequired")),
  ...
  const handleDocumentInputMethodChange = (option, setFieldValue) => {
    setDocumentInputMethod(option?.value);
  ...
  const triggerFileUpload = () => {
    ...
    if (selectedType?.value === "document" && !documentInputMethod) {
      toast.error(t("mediaLibrary.validation.selectUploadMethodFirst"));
      return;
    }
  ...
  const shouldShowInputMethod = () => {
    return (
      selectedType?.value === "video" || selectedType?.value === "document"
    );
  };

  const getCurrentInputMethod = () => {
    if (selectedType?.value === "video") return videoInputMethod;
    if (selectedType?.value === "document") return documentInputMethod;
    return null;
  };
  ...
  const shouldShowFileUpload = () => {
    ...
    if (selectedType?.value === "h5p") return false;
    ...
    return selectedType?.value === "image" || selectedType?.value === "audio";
  };

  const shouldShowUrlInput = () => {
    if (selectedType?.value === "video" && videoInputMethod === "url")
      return true;
    if (selectedType?.value === "document" && documentInputMethod === "url")
      return true;
    // Always show URL input for H5P type
    if (selectedType?.value === "h5p") return true;
    return false;
  };
              ...
              <Form onSubmit={handleSubmit} className="space-y-8">
                ...
                {shouldShowInputMethod() && (
                  <div>
                    ...
                    <SelectField
                      ...
                      onChange={(option) => {
                        if (selectedType?.value === "video") {
                          ...
                          handleDocumentInputMethodChange(
                            option,
                            setFieldValue,
                          );
                        }
                    ...
                    {selectedType?.value === "document" &&
                      touched.documentInputMethod &&
                      errors.documentInputMethod && (
                        <p className="text-red-500 text-sm mt-1">
                          {errors.documentInputMethod}
                        </p>
                      )}
