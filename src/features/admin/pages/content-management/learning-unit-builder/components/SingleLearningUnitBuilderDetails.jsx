...
const SingleLearningUnitBuilderDetails = () => {
  ...
  // Fetch content data only initially
  useEffect(() => {
    if (id && topicId) {
      dispatch(getContentById({ topicId, id }));
    }
  ...
  // Set initial selected values from content and start sequential loading
  useEffect(() => {
    if (content?.topic) {
      const programVal = {
        label: content.topic.program.title,
  ...
  // STEP 1: Load programs after content is loaded (only once)
  useEffect(() => {
  ...
  // STEP 2: Load levels when program is selected (but not loaded yet)
  useEffect(() => {
  ...
  // STEP 3: Load modules when level is selected (but not loaded yet)
  useEffect(() => {
  ...
  // STEP 4: Load chapters when module is selected (but not loaded yet)
  useEffect(() => {
  ...
  // STEP 5: Load topics when chapter is selected (but not loaded yet)
  useEffect(() => {
                        ...
                        <div>
                          <SelectField
                            ...
                            onChange={(option) => {
                              ...
                              setSelectedChapter(null);
                              setSelectedTopic(null);
                              // Reset all dependent loaded states
