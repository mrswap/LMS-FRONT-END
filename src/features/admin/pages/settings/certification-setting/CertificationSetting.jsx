...
const CertificationSetting = () => {
  ...
  useEffect(() => {
    ...
    // Handle logo URL from API response
    if (settings?.data?.company_logo_url) {
      setLogoPreview(settings.data.company_logo_url);
    } else if (settings?.data?.company_logo) {
      setLogoPreview(settings.data.company_logo);
    }
    // Handle signature URL from API response
    if (settings?.data?.signer_signature_url) {
      setSignaturePreview(settings.data.signer_signature_url);
    } else if (settings?.data?.signature) {
      setSignaturePreview(settings.data.signature);
    }
  }, [quill, settings?.data]);
  ...
  const handleSignatureUpload = (event, setFieldValue) => {
    ...
    if (file) {
      if (file.size > 2 * 1024 * 1024) {
        toast.error(t("certificateSetting.errors.signatureSize"));
        return;
      }
      if (!file.type.match(/image\/(jpeg|png|jpg|webp)/)) {
        toast.error(t("certificateSetting.errors.signatureType"));
        return;
      }
  ...
  const handleRemoveSignature = (setFieldValue) => {
  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    try {
      ...
      // Append logo file if new file is selected
      if (logoFile) {
      ...
      // Append signature file if new file is selected
      if (signatureFile) {
        formData.append("signer_signature", signatureFile);
      } else {
        formData.append("signer_signature", "");
      }
          ...
          {({ isSubmitting, values, setFieldValue, handleSubmit }) => (
            <Form onSubmit={handleSubmit}>
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                ...
                {/* Main Content Area */}
                <div className="md:col-span-8 xl:col-span-9 space-y-6">
                  {/* Basic Information with Logo & Signature */}
                  <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
                    <div className="border-b border-gray-200 px-6 py-4 bg-gray-50">
                      <h3 className="font-medium text-gray-900">
                        {t("certificateSetting.sections.certificateInfo")}
                    ...
                    <div className="p-6">
                      ...
                      <div className="mb-6 pb-6 border-b border-gray-200">
                        <label className="block text-sm font-medium text-gray-700 mb-3">
                          {t("certificateSetting.fields.signature")}
                        </label>
                        <div className="flex items-center gap-6">
                          {signaturePreview ? (
                            <>
                              <div className="relative group">
                                <div className="w-32 h-20 border border-gray-200 rounded-lg p-2 bg-gray-50 flex items-center justify-center">
                                  <img
                                ...
                                <button
                                  type="button"
                                  onClick={() =>
                                    handleRemoveSignature(setFieldValue)
                                  }
                                  className="absolute -top-2 -right-2 p-1 bg-white border border-gray-300 rounded-full shadow-sm hover:bg-gray-50 transition-all"
                                >
                                  <FiTrash2
                              ...
                              >
                                {t(
                                  "certificateSetting.buttons.changeSignature",
                                )}
                              </button>
                            </>
                          ) : (
                            ...
                            >
                              ...
                              <p className="text-sm text-gray-600">
                                {t(
                                  "certificateSetting.buttons.uploadSignature",
                                )}
                              </p>
                              <p className="text-xs text-gray-400 mt-1">
                                {t("certificateSetting.signatureConstraints")}
                              </p>
                            </div>
                          )}
                          <input
                            ...
                            accept="image/jpeg,image/png,image/jpg,image/webp"
                            onChange={(e) =>
                              handleSignatureUpload(e, setFieldValue)
                            }
                            className="hidden"
                          />
                        </div>
